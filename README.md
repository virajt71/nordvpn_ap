...
- **Rust API Orchestrator**: A lightweight Axum backend that programmatically manages access point sibling stacks on the host via Docker with built-in security headers (`nosniff`, `X-Frame-Options: DENY`, `X-XSS-Protection`).
- **Technical Glassmorphism Dashboard**: A premium, responsive dark-mode dashboard (`static/` HTML/CSS/JS with `Fira Code` / `Fira Sans` technical typography) served directly by the orchestrator.
- **Dashboard Telemetry Summary Bar**: Real-time overview metrics bar displaying active stack ratios, VPN tunneled gateways count, and discovered system hardware interfaces.
- **Live WebSocket Streaming**: Real-time status streaming over `/ws/stacks` for instant UI updates when stacks start, stop, restart, or change state.
- **Container Log Viewer Modal**: Tabbed log terminal interface allowing inspection of `Gluetun (VPN)`, `WiFi-AP (hostapd/dnsmasq)`, and `AdGuard Home` container logs directly from the browser.
...
- **API Security & Management**: REST management API with security headers. *Run on a trusted network or behind an authenticating reverse proxy.*
